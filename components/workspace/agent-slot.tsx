...
export function AgentSlot({
  ...
  return (
    <UICard className="border-teal-200/90 bg-white shadow-xs overflow-hidden ring-1 ring-teal-500/10">
      <UICardHeader className="border-b border-teal-100 bg-gradient-to-r from-teal-50 via-teal-50/50 to-white py-4 px-5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <UICardTitle className="text-sm font-bold text-slate-900 flex items-center gap-1.5">
                Follow-up AI Assistant
              </UICardTitle>
              <p className="text-[11px] text-teal-800 font-medium">Stateful Care Coordinator</p>
            </div>
          </div>
          <Badge variant="default" className="text-[10px] px-2 py-0.5 bg-teal-100 text-teal-800 font-semibold border-teal-200">
            Agent Connected
          </Badge>
        </div>
      </UICardHeader>
      ...
      <UICardContent className="p-4 sm:p-5 space-y-4">
        ...
        <div className="rounded-xl bg-slate-50/80 p-3.5 border border-slate-200/80 text-xs text-slate-600 space-y-1.5">
        ...
        <div className="rounded-xl border border-teal-100 bg-teal-50/40 p-3 text-xs space-y-2">
          ...
          <div className="grid grid-cols-3 gap-1.5 text-center font-mono text-[10px]">
            <div className="rounded-lg bg-white/80 p-1.5 border border-teal-100">
              <div className="font-bold text-slate-800">{patientContext.tasks.length}</div>
              <div className="text-slate-500 text-[9px]">Tasks</div>
            </div>
            <div className="rounded-lg bg-white/80 p-1.5 border border-teal-100">
              <div className="font-bold text-slate-800">{patientContext.appointments.length}</div>
              <div className="text-slate-500 text-[9px]">Visits</div>
            </div>
            <div className="rounded-lg bg-white/80 p-1.5 border border-teal-100">
              <div className="font-bold text-slate-800">{patientContext.reminders.length}</div>
              <div className="text-slate-500 text-[9px]">Reminders</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-1.5">
            {samplePrompts.map((prompt) => (
              <div
                key={prompt}
                className="group flex items-center justify-between rounded-lg border border-slate-200/80 bg-white px-3 py-2 text-xs text-slate-700 transition-all hover:border-teal-300 hover:bg-teal-50/30 cursor-pointer shadow-2xs"
              >
                <span className="text-[11px]">{prompt}</span>
                <ArrowUpRight className="h-3 w-3 text-slate-400 group-hover:text-teal-700 transition-colors" />
              </div>
            ))}
          </div>
        </div>
